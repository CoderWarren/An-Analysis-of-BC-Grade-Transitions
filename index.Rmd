...
```{r heaty, echo=FALSE, message=FALSE, warning=FALSE, fig.cap="Figure 4: Heatmap displaying the trend in overall transition rates for all Grade 11 students for each district over the years. (It is very likely that I will delete or tweak this heatmap prior to the submission of my final project. I really want to add a dropdown to it so I can display data from all grades all at once.)"}
district_mat <- data_district %>%
  filter(SUB_POPULATION == "All Students") %>%
  filter(grade == "11") %>%
  select(DISTRICT_NAME, year, rate) %>%
  mutate(DISTRICT_NAME = as.factor(DISTRICT_NAME))

district_mat2 <- as.data.frame(pivot_wider(district_mat, names_from=DISTRICT_NAME,
                                            values_from = rate))

district_mat2 %>%
  column_to_rownames("year") %>%
  as.matrix() -> district_mat2

plot_ly(x=colnames(district_mat2), 
        y=rownames(district_mat2), 
        z=~district_mat2, type="heatmap",
        colorbar = list(title = "<b>Rate of Transitions</b>")) %>%
  layout(
    title="Rate of successful grade transitions (Grade 11 Students)",
...
```{r, echo=FALSE, eval=FALSE}
steps <- list(
  list(args = list("transforms[1].value", unique(data_district$year)[1]),
                    label = unique(data_district$year)[1],
                    method = "restyle", 
                    value = "1"),
  list(args = list("transforms[1].value", unique(data_district$year)[2]),
                    label = unique(data_district$year)[2],
                    method = "restyle", 
                    value = "2"),
  list(args = list("transforms[1].value", unique(data_district$year)[3]),
                    label = unique(data_district$year)[3],
                    method = "restyle", 
                    value = "3"
                    )
  )
...
fig <- data_district %>% 
  ...
  plot_ly(type = 'scattergeo', 
          ...
          color= ~rate,
          text = ~paste('</br> Year: ', year,
                        '</br> District Number: ', DISTRICT_NUMBER,
                        '</br> District Name: ', DISTRICT_NAME,
                        '</br> District City: ', DISTRICT_CITY,
                        '</br> Grade: ', grade,
          ...
          transforms=list(
            ...
            list(
              ...
              target = 'customdata',
              operation = '=',
              value = unique(data_district$year)[1]
            )
            )) %>%
  layout(geo = g,
         annotations = list(text = "<b> Grade: </b>",  x = 0.9, xref="paper",
         ...
         updatemenus = list(
           list(
             ...
             buttons=list(
               list(method = "restyle",
                    args = list("transforms[0].value", unique(data_district$grade)[1]),
                    label = unique(data_district$grade)[1]),
               list(method = "restyle",
                    args = list("transforms[0].value", unique(data_district$grade)[2]),
                    label = unique(data_district$grade)[2]),
               list(method = "restyle",
                    args = list("transforms[0].value", unique(data_district$grade)[3]),
                    label = unique(data_district$grade)[3]),
               list(method = "restyle",
                    args = list("transforms[0].value", unique(data_district$grade)[4]),
                    label = unique(data_district$grade)[4]),
               list(method = "restyle",
                    args = list("transforms[0].value", unique(data_district$grade)[5]),
                    label = unique(data_district$grade)[5]),
               list(method = "restyle",
                    args = list("transforms[0].value", unique(data_district$grade)[6]),
                    label = unique(data_district$grade)[6]),
               list(method = "restyle",
                    args = list("transforms[0].value", unique(data_district$grade)[7]),
                    label = unique(data_district$grade)[7]),
               list(method = "restyle",
                    args = list("transforms[0].value", unique(data_district$grade)[8]),
                    label = unique(data_district$grade)[8]),
               list(method = "restyle",
                    args = list("transforms[0].value", unique(data_district$grade)[9]),
                    label = unique(data_district$grade)[9]),
               list(method = "restyle",
                    args = list("transforms[0].value", unique(data_district$grade)[10]),
                    label = unique(data_district$grade)[10]),
               list(method = "restyle",
                    args = list("transforms[0].value", unique(data_district$grade)[11]),
                    label = unique(data_district$grade)[11])
               )
             )
           ),
...
```{r YEEETER, echo=FALSE, eval=FALSE, message=FALSE, warning=FALSE, fig.cap="Figure 7: Interative map displaying the location of the board offices for each district. The color of the points is based on the rate of successful transitions from Grade 11 to 12 in 2020."}
